...
import { previewReporte606, generarTxt606, generarCsv606 } from "./actions";
import type { ReporteDgiiGenerado } from "@/lib/supabase/types";

export const metadata = { title: "606 · Compras — DGII" };

export default async function Reporte606Page({
  searchParams,
}: {
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6 md:px-8 md:py-8">
      ...
      <FadeIn>
        <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight md:text-3xl">
          <FileSpreadsheet className="h-7 w-7" /> Formato 606 — Compras
        </h1>
        <p className="mb-6 text-sm text-muted-foreground">
          Reporte mensual de compras de bienes y servicios. Cada compra de oro
          se incluye como &quot;Tipo Bien 09 — Costo de Venta&quot;, forma de
          pago efectivo.
        </p>
      </FadeIn>
      ...
      <GeneradorReporte
        ...
        onGenerarTxt={generarTxt606}
        onGenerarCsv={generarCsv606}
      />
            ...
            <table className="w-full min-w-[900px] border-separate border-spacing-0 text-xs">
              <thead>
                <tr className="bg-muted/40 text-left uppercase tracking-wide text-muted-foreground">
                  ...
                  <th className="p-2 text-right">Bienes</th>
              ...
              <tbody>
                {preview.registros.map((r, i) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="p-2">{r.tipoBien}</td>
                    <td className="p-2">
                      {formatearFechaCorta(
                        typeof r.fechaComprobante === "string"
                          ? r.fechaComprobante
                          : r.fechaComprobante.toISOString(),
                      )}
                    </td>
                    <td className="p-2 text-right tabular-nums">
                      {formatearDOP(Number(r.montoBienes ?? 0))}
                    </td>
                    <td className="p-2 text-right tabular-nums font-semibold">
                      {formatearDOP(
                        Number(
                          r.totalFacturado ?? (r.montoBienes ?? 0),
                        ),
                      )}
                    </td>
                    <td className="p-2">{r.formaPago}</td>
                  </tr>
                ))}
              </tbody>
            </table>
